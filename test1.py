...
cap = cv2.VideoCapture(0) # 이건 윈도우 용
# cap = cv2.VideoCapture('/dev/video0') # 이건 라즈베리파이 용
...
while True:
    ...
    # PoseDetector로부터 사람 위치 정보 얻기
    imList, bboxs = detector.findPosition(img, draw=True, bboxWithHands=False)

    if success:
        if bboxs:  # bboxs 리스트에 사람이 감지된 경우
            bbox = bboxs
            # bbox = bboxs[0]으로 해야 한 사람만 인식하는데, [0]을 쓰니까 코드 실행이 안됐어요.
            # 왜지...
            center = bbox["center"]
            x, y, w, h = bbox['bbox']
            ...
            cv2.putText(img, "no", (20, 100), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)
            # motor1.stop()
            # motor2.stop()
    else:
        cv2.putText(img, "사람이 감지되지 않았습니다.", (20, 100), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 255), 2)
